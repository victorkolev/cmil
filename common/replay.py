...
class Replay:
    def __init__(
        ...
        self._complete_eps = load_episodes(self._directory, capacity, read_fn)
    ...
    def _generate_chunks(self, length):
        ...
        while True:
            ...
            if False: # random shift
                assert chunk['image'].shape[1:] == (64, 64, 3), chunk['image'].shape
                img = np.pad(chunk['image'], ((0, 0), (3, 3), (3, 3), (0, 0)), 'edge')
                x_coord = np.random.randint(0, 6)
                y_coord = np.random.randint(0, 6)
                chunk['image'] = img[:, x_coord:x_coord+64, y_coord:y_coord+64]
            yield chunk
...
default_keys = ['image', 'action', 'reward', 'is_first', 'is_last', 'is_terminal']
def read_episode(f, keys=default_keys):
    ep = np.load(f)
    episode = {
        k: ep[k]
        for k in keys
    ...
    if episode['image'].shape[1:3] == (128, 128):
        episode['image'] = episode['image'][:, ::2, ::2]

    assert episode['image'].shape[1:] == (64, 64, 3), episode['image'].shape
    return episode


def load_episodes(directory, capacity=None, read_fn=None):
    # The returned directory from filenames to episodes is guaranteed to be in
    # temporally sorted order.
    if read_fn is None:
        read_fn = read_episode
