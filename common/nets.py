...
class EnsembleRSSM(common.Module):
    ...
    def observe(self, embed, action, is_first, state=None):
        ...
        prior["ensemble_mean"] = tf.repeat(
            tf.expand_dims(prior["mean"], axis=-2), repeats=self._ensemble, axis=-2
        )
        prior["ensemble_std"] = tf.repeat(
            tf.expand_dims(prior["std"], axis=-2), repeats=self._ensemble, axis=-2
        )
    ...
    def img_step(self, prev_state, prev_action, sample=True):
        ...
        ensemble, stats = self._suff_stats_ensemble(x)
        ensemble = {"ensemble_" + k: v for k, v in ensemble.items()}
        # index = tf.random.uniform((), 0, self._ensemble, tf.int32)
        # stats = {k: v[index] for k, v in stats.items()}
        dist = self.get_dist(stats)
        stoch = dist.sample() if sample else dist.mode()
        prior = {"stoch": stoch, "deter": deter, **stats, **ensemble}
        return prior
    ...
    def _suff_stats_ensemble(self, inp):
        ...
        stats = {k: tf.stack([x[k] for x in stats], -2) for k, v in stats[0].items()}
        stats = {
            k: v.reshape(bs + [v.shape[-2]] + list(v.shape[2:]))
